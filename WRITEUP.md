...
**Components:**
1. **Query Parser :** A local LLM (Llama 3B) instructed via a strict system prompt to translate natural language queries into a JSON filter. It handles exact constraints (e.g., employee count boundaries, dynamic mapping of geographical regions to ISO-2 codes). With proper fine-tuning and distilation of the model, it can predict more fields like target_markets or business_model.
2. **Data Engine :** A custom Python engine that reads a `.jsonl` file line-by-line. It cleans stringified nested objects (like `address`), intelligently merges duplicate entries (filling `None` values and combining arrays using `operational_name` or `website` as keys), and applies the LLM's JSON filter recursively.
...
* **Latency vs. Simplicity:** Parsing the dataset line-by-line in Python is O(N). I traded query speed (which would be faster with a pre-indexed database) for architectural simplicity and memory efficiency, avoiding the need to spin up and maintain a dedicated database instance for the initial iteration. This can be of course solved without too much headache.
...
1. **Negation handling:** If a user searches for "Companies doing packaging for cosmetics", and a company description states "We provide packaging for food, but NOT cosmetics", the LLM's `$contains` filter will find both words and pass it to the NLP ranker. Semantic models often struggle with negation, so this company might still receive a high relevance score. This can be managed with proper fine-tuning for this specific task.
...
3. **NLP Component:** The Ranker architecture would remain the same. Because the database handles the heavy lifting of hard filtering, the NLP model still only needs to evaluate a small subset of results (e.g., the top 100 matches), allowing the semantic search to remain fast and computationally cheap.
